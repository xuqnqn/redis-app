...
CThreadNotify CacheManager::s_free_notify;
...
int CacheConn::Init()
{
    ...
    if (!m_pContext || m_pContext->err) {
        if (m_pContext) {
            printf("redisConnect failed: %s", m_pContext->errstr);
            redisFree(m_pContext);
            m_pContext = NULL;
        } else {
    ...
    redisReply* reply = (redisReply *)redisCommand(m_pContext, "AUTH %s", m_pCachePool->GetServerAuth());
...
long CacheConn::incrBy(string key, long value)
{
    ...
    if(!reply)
    {
        printf("redis Command failed:%s", m_pContext->errstr);
        redisFree(m_pContext);
        m_pContext = NULL;
        return -1;
    }
...
///////////////
CachePool::CachePool(const char* pool_name, const char* server_ip, const char* server_auth, int server_port, int db_num, int max_conn_cnt)
{
...
int CachePool::Init()
{
    ...
    printf("cache pool: %s, list size: %lu", m_pool_name.c_str(), m_free_list.size());
    return 0;
}
    ...
    while (m_free_list.empty()) {
        if (m_cur_conn_cnt >= m_max_conn_cnt) {
            ...
            if (ret) {
                ...
                m_cur_conn_cnt++;
                printf("new cache connection: %s, conn_cnt: %d", m_pool_name.c_str(), m_cur_conn_cnt);
            }
        }
    }
...
int CacheManager::Init()
{
    ...
    if (!cache_instances) {
        printf("not configure CacheIntance");
        return 1;
    }
    ...
    for (uint32_t i = 0; i < instances_name.GetItemCnt(); i++) {
        ...
        char* cache_auth = config_file.GetConfigName(auth);
        char* str_cache_port = config_file.GetConfigName(port);
        char* str_cache_db = config_file.GetConfigName(db);
        char* str_max_conn_cnt = config_file.GetConfigName(maxconncnt);
        if (!cache_host || !cache_auth || !str_cache_port || !str_cache_db || !str_max_conn_cnt) {
            printf("not configure cache instance: %s", pool_name);
            return 2;
        }

        CachePool* pCachePool = new CachePool(pool_name, cache_host, cache_auth, atoi(str_cache_port),
