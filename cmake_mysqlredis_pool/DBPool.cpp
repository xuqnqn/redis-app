...
CThreadNotify CDBManager::s_free_notify;
...
int CDBManager::Init()
{
    ...
    if (!db_instances) {
        printf("not configure DBInstances");
        return 1;
    }
    ...
    for (uint32_t i = 0; i < instances_name.GetItemCnt(); i++) {
        ...
        if (!db_host || !str_db_port || !db_dbname || !db_username || !db_password || !str_maxconncnt) {
            printf("not configure db instance: %s", pool_name);
            return 2;
        }
        ...
        if (pDBPool->Init()) {
            printf("init db instance failed: %s", pool_name);
            return 3;
        }
