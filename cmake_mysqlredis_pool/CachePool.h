...
class CachePool {
public:
    CachePool(const char* pool_name, const char* server_ip, const char* server_auth, int server_port, int db_num, int max_conn_cnt);
    virtual ~CachePool();
    ...
    const char* GetServerAuth() { return m_server_auth.c_str(); }
...
private:
    ...
    static CThreadNotify    s_free_notify;
